...
```{r}
student_av <- grades |> select(!semester) |> group_by(id, course, group) |> summarise(`Average per course` = mean(grade), .groups = "drop_last") |> pivot_wider(!id & !group, names_to = course)
```
